Goal
- Convert /home/user/.library-catalog/shell/michaelshimeles.ralphy/ralphy.sh into a Bun CLI using yargs with full flag parity, preserving behavior for Claude/OpenCode/Cursor/Codex/Qwen/Droid.
...
- [ ] Task 1: Create Bun CLI entrypoint with yargs parsing, help/version output, and option schema parity (no behavior yet).
- [ ] Task 2: Add server bootstrap (Bun.serve) with `/v1/health` and `/v1/version`.
- [ ] Task 3: Implement CLI -> server request dispatch (spawns server, waits for ready, calls endpoint, shuts down).
- [ ] Task 4: Add types for CLI options and server requests/responses.
...
- [ ] Task 1: Implement `.ralphy/` init logic (project detection, config.yaml + progress.txt creation, overwrite prompt).
- [ ] Task 2: Implement config read/show and `--add-rule` update with YAML manipulation.
- [ ] Task 3: Wire CLI flags (`--init`, `--config`, `--add-rule`) to server endpoints.
- [ ] Task 4: Add unit tests for config init/show/add-rule.
- Demo/verify: run `--init`, inspect `.ralphy/config.yaml`, `--config`, `--add-rule`.

Sprint 3: PRD sources + task model
- [ ] Task 1: Implement markdown PRD parsing (`- [ ]` and `- [x]`), task completion updates with safe escaping.
- [ ] Task 2: Implement YAML task parsing (`tasks[]`, `completed`, `parallel_group`) and completion updates.
- [ ] Task 3: Implement GitHub issues source using `gh issue list/view/close` with label filter.
- [ ] Task 4: Add unified task source interface + `GET /v1/tasks/next` and `POST /v1/tasks/complete`.
- [ ] Task 5: Add tests for all task sources with fixtures.
- Demo/verify: `--prd`, `--yaml`, `--github` + `tasks/next` and completion updates.

Sprint 4: Prompt builders + agent runners (single-task)
- [ ] Task 1: Build prompt construction parity (config rules/boundaries/project context, progress file, test/lint flags, commit instructions, completion tag).
- [ ] Task 2: Implement agent runner adapters for Claude/OpenCode/Cursor/Codex/Qwen/Droid via Bun.spawn.
- [ ] Task 3: Implement single-task (brownfield) run flow, retries, and result parsing.
- [ ] Task 4: Add tests for prompt assembly and command construction (snapshot style).
- Demo/verify: run positional task with each engine flag in dry-run and real modes.

Sprint 5: PRD sequential loop
- [ ] Task 1: Implement requirement checks (git repo, dependencies, PRD presence).
- [ ] Task 2: Implement sequential loop: select next task, run agent, update PRD, track retries and max-iterations.
- [ ] Task 3: Implement progress logging + cost/token accounting.
- [ ] Task 4: Add tests for loop control paths (max-iterations, no tasks, retry exhaustion).
- Demo/verify: run PRD loop against markdown/YAML sources with `--max-iterations`.

Sprint 6: Git workflows + PR creation
- [ ] Task 1: Implement branch-per-task flow (stash handling, slugify, branch naming).
- [ ] Task 2: Implement `--create-pr` + `--draft-pr` using `gh`.
- [ ] Task 3: Add tests for branch naming, command construction, and dry-run flows.
- Demo/verify: run PRD loop with `--branch-per-task` and `--create-pr` in a test repo.

Sprint 7: Parallel mode + worktrees
- [ ] Task 1: Implement worktree creation per task group with isolated dirs and copy PRD.
- [ ] Task 2: Implement parallel execution pooling with `--max-parallel`.
- [ ] Task 3: Implement integration branch merges and conflict resolution flow (AI-assisted) parity.
- [ ] Task 4: Cleanup/trap handling for worktrees and background processes.
- [ ] Task 5: Add tests for worktree lifecycle and merge command construction.
